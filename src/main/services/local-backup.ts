...
const resolveBackupFile = (localBackupPath: string, fileName: string) => {
  // Artifact names originate in the folder listing, but callers can invoke
  // the IPC endpoint directly. Refuse traversal and absolute paths so a
  // malformed renderer request cannot read or delete files outside the folder.
  if (
    !fileName ||
    path.basename(fileName) !== fileName ||
    fileName.includes("\\") ||
    !/^(steam|custom)-[^/]+-\\d+\\.tar$/.test(fileName)
  ) {
    throw new Error("Invalid local backup file name");
  }

  const basePath = path.resolve(localBackupPath);
  const resolvedPath = path.resolve(basePath, fileName);
  if (!resolvedPath.startsWith(`${basePath}${path.sep}`)) {
    throw new Error("Invalid local backup file path");
  }
...
export class LocalBackupService {
  ...
  public static async downloadBackup(
    ...
    const tarPath = resolveBackupFile(localBackupPath, fileName);
    const metaPath = `${tarPath}.meta.json`;
    const backupDir = path.join(backupsPath, `${shop}-${objectId}`);
