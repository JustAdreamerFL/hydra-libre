...
export class HydraApi {
  ...
  static async handleExternalAuth(uri: string) {
    ...
    // Merge the selected local profile folder as soon as the account is
    // known. This keeps profile metadata and achievements available even when
    // the remote API is unavailable.
    await ProfileSyncService.syncProfile().catch((error) => {
      logger.warn("Profile folder sync after sign-in was skipped:", error);
    });
