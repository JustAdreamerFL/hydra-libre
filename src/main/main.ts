...
export const loadState = async () => {
  ...
  try {
    ...
    // Profile-folder sync is free and local-first. It is best-effort during
    // startup so a missing/unavailable sync drive never prevents the launcher
    // from opening offline.
    void ProfileSyncService.syncProfile().catch((error) => {
      logger.warn("Profile folder sync was skipped:", error);
    });
